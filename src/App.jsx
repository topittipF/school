/* ========================================
   SCHOOL STEP TO SUCCESS — FULL CLOUD LMS
   ======================================== */
...
const FirebaseDB = {
  ...
  async loadWhere(col, field, op, value) {
...
function App() {
  useEffect(() => {
    const loadInitial = async () => {
      ...
      grades = await FirebaseDB.load(COLLECTIONS.grades);
      assignments = await FirebaseDB.load(COLLECTIONS.assignments);
      onlineLessons = await FirebaseDB.load(COLLECTIONS.onlineLessons);
      units = await FirebaseDB.load(COLLECTIONS.units);
      const sched = await FirebaseDB.load(COLLECTIONS.schedule);
      schedule = sched[0]?.data || {};
    };
    loadInitial();
  }, []);
